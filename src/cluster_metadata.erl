...
-type iterator()            :: {cluster_metadata_manager:metadata_iterator(), it_opts()}.
...
get({Prefix, SubPrefix}=FullPrefix, Key, Opts)
  when (is_binary(Prefix) orelse is_atom(Prefix)) andalso
    ...
    case cluster_metadata_manager:get(PKey) of
        undefined -> Default;
        Existing ->
...
iterator({Prefix, SubPrefix}=FullPrefix, Opts)
  when (is_binary(Prefix) orelse is_atom(Prefix)) andalso
    ...
    It = cluster_metadata_manager:iterator(FullPrefix, KeyMatch),
    {It, Opts}.

%% @doc Advances the iterator
-spec itr_next(iterator()) -> iterator().
itr_next({It, Opts}) ->
    It1 = cluster_metadata_manager:iterate(It),
    {It1, Opts}.
...
itr_done({It, _Opts}) ->
    cluster_metadata_manager:iterator_done(It).
...
itr_key_values({It, Opts}) ->
    ...
    {Key, Obj} = cluster_metadata_manager:iterator_value(It),
    AllowPut = get_option(allow_put, Opts, true),
    case get_option(resolver, Opts, undefined) of
        undefined ->
            {Key, maybe_tombstones(cluster_metadata_object:values(Obj), Default)};
        Resolver ->
            Prefix = cluster_metadata_manager:iterator_prefix(It),
...
itr_key({It, _Opts}) ->
    {Key, _} = cluster_metadata_manager:iterator_value(It),
    Key.
...
itr_values({It, Opts}) ->
    Default = itr_default({It, Opts}),
    {_, Obj} = cluster_metadata_manager:iterator_value(It),
    maybe_tombstones(cluster_metadata_object:values(Obj), Default).
...
itr_value({It, Opts}) ->
    ...
    case get_option(resolver, Opts, undefined) of
        undefined ->
            case cluster_metadata_object:value_count(Obj) of
                1 ->
                    maybe_tombstone(cluster_metadata_object:value(Obj), Default);
                _ ->
                    {error, conflict}
            end;
        Resolver ->
...
prefix_hash(Prefix) when is_tuple(Prefix) or is_atom(Prefix) or is_binary(Prefix) ->
    cluster_metadata_hashtree:prefix_hash(Prefix).
...
put({Prefix, SubPrefix}=FullPrefix, Key, ValueOrFun, _Opts)
  when (is_binary(Prefix) orelse is_atom(Prefix)) andalso
    ...
    Updated = cluster_metadata_manager:put(PKey, CurrentContext, ValueOrFun),
...
current_context(PKey) ->
    case cluster_metadata_manager:get(PKey) of
        undefined -> cluster_metadata_object:empty_context();
        CurrentMeta -> cluster_metadata_object:context(CurrentMeta)
    end.

%% @private
maybe_resolve(PKey, Existing, Method, AllowPut) ->
    SibCount = cluster_metadata_object:value_count(Existing),
    maybe_resolve(PKey, Existing, SibCount, Method, AllowPut).

%% @private
maybe_resolve(_PKey, Existing, 1, _Method, _AllowPut) ->
    cluster_metadata_object:value(Existing);
maybe_resolve(PKey, Existing, _, Method, AllowPut) ->
    Reconciled = cluster_metadata_object:resolve(Existing, Method),
    RContext = cluster_metadata_object:context(Reconciled),
    RValue = cluster_metadata_object:value(Reconciled),
    case AllowPut of
        false ->
            ok;
        true ->
            Stored = cluster_metadata_manager:put(PKey, RContext, RValue),
...
broadcast(PKey, Obj) ->
    Broadcast = #metadata_broadcast{pkey = PKey,
                                    obj  = Obj},
    cluster_metadata_broadcast:broadcast(Broadcast, cluster_metadata_manager).
