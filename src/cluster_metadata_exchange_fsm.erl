...
%%
%% -------------------------------------------------------------------
-module(cluster_metadata_exchange_fsm).
...
prepare(start, State) ->
    %% get local lock
    case cluster_metadata_hashtree:lock() of
        ok ->
            %% get remote lock
...
exchange(timeout, State=#state{peer=Peer}) ->
    RemoteFun = fun(Prefixes, {get_bucket, {Level, Bucket}}) ->
                        cluster_metadata_hashtree:get_bucket(Peer, Prefixes, Level, Bucket);
                   (Prefixes, {key_hashes, Segment}) ->
                        cluster_metadata_hashtree:key_hashes(Peer, Prefixes, Segment)
                end,
    HandlerFun = fun(Diff, Acc) ->
                         repair(Peer, Diff),
                         track_repair(Diff, Acc)
                 end,
    Res = cluster_metadata_hashtree:compare(RemoteFun, HandlerFun,
...
repair_sub_prefixes(Type, Peer, Prefix, It) ->
    case cluster_metadata_manager:iterator_done(It) of
        true ->
            cluster_metadata_manager:iterator_close(It);
        false ->
            SubPrefix = cluster_metadata_manager:iterator_value(It),
            FullPrefix = {Prefix, SubPrefix},
            ...
            repair_sub_prefixes(Type, Peer, Prefix,
                                cluster_metadata_manager:iterate(It))
    end.

%% @private
repair_full_prefix(Type, Peer, FullPrefix, ObjIt) ->
    case cluster_metadata_manager:iterator_done(ObjIt) of
        true ->
            cluster_metadata_manager:iterator_close(ObjIt);
        false ->
            {Key, Obj} = cluster_metadata_manager:iterator_value(ObjIt),
            repair_other(Type, Peer, {FullPrefix, Key}, Obj),
            repair_full_prefix(Type, Peer, FullPrefix,
                               cluster_metadata_manager:iterate(ObjIt))
    end.
...
repair_keys(Peer, PrefixList, {_Type, KeyBin}) ->
    ...
    PKey = {Prefix, Key},
    LocalObj = cluster_metadata_manager:get(PKey),
    RemoteObj = cluster_metadata_manager:get(Peer, PKey),
...
merge(undefined, PKey, RemoteObj) ->
    cluster_metadata_manager:merge({PKey, undefined}, RemoteObj);
merge(Peer, PKey, LocalObj) ->
    cluster_metadata_manager:merge(Peer, {PKey, undefined}, LocalObj).


%% @private
repair_iterator(local, _, Prefix) when is_atom(Prefix) orelse is_binary(Prefix) ->
    cluster_metadata_manager:iterator(Prefix);
repair_iterator(local, _, Prefix) when is_tuple(Prefix) ->
    cluster_metadata_manager:iterator(Prefix, undefined);
repair_iterator(remote, Peer, PrefixOrFull) ->
    cluster_metadata_manager:remote_iterator(Peer, PrefixOrFull).
...
remote_lock_request(Peer) ->
    Self = self(),
    as_event(fun() ->
                     Res = cluster_metadata_hashtree:lock(Peer, Self),
                     {remote_lock, Res}
             end).
...
update_request(Node) ->
    as_event(fun() ->
                     ...
                     %% and tree is built
                     case cluster_metadata_hashtree:update(Node) of
                         ok -> tree_updated;
