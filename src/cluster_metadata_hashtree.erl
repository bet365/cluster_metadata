...
%%
%% -------------------------------------------------------------------
-module(cluster_metadata_hashtree).
...
%% @private
build() ->
    PrefixIt = cluster_metadata_manager:iterator(),
    build(PrefixIt).

%% @private
build(PrefixIt) ->
    case cluster_metadata_manager:iterator_done(PrefixIt) of
        true ->
            cluster_metadata_manager:iterator_close(PrefixIt);
        false ->
            Prefix = cluster_metadata_manager:iterator_value(PrefixIt),
            ObjIt = cluster_metadata_manager:iterator(Prefix, undefined),
...
build(PrefixIt, ObjIt) ->
    case cluster_metadata_manager:iterator_done(ObjIt) of
        true ->
            ...
            build(cluster_metadata_manager:iterate(PrefixIt));
        false ->
            FullPrefix = cluster_metadata_manager:iterator_prefix(ObjIt),
            {Key, Obj} = cluster_metadata_manager:iterator_value(ObjIt),
            Hash = cluster_metadata_object:hash(Obj),
            %% insert only if missing to not clash w/ newer writes during build
            ?MODULE:insert({FullPrefix, Key}, Hash, true),
            build(PrefixIt, cluster_metadata_manager:iterate(ObjIt))
    end.
