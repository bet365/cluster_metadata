-module(cluster_metadata_manager).

-behaviour(gen_server).
-behaviour(cluster_metadata_broadcast_handler).
...
%% cluster_broadcast_handler callbacks
...
%% @doc Start cluster_metadadata_manager and link to calling process.
%%
...
%% cluster_metadata_object:context/1. Values can obtained w/ cluster_metadata_object:values/1.
...
%% @doc Deconstructs are broadcast that is sent using `cluster_metadata_manager' as the
...
broadcast_data(#metadata_broadcast{pkey=Key, obj=Obj}) ->
    Context = cluster_metadata_object:context(Obj),
    {{Key, Context}, Obj}.
...
graft(Context, Obj) ->
    case cluster_metadata_object:equal_context(Context, Obj) of
        false ->
...
handle_call({is_stale, PKey, Context}, _From, State) ->
    Existing = read(PKey),
    IsStale = cluster_metadata_object:is_stale(Context, Existing),
    {reply, IsStale, State}.
...
read_modify_write(PKey, Context, ValueOrFun, State=#state{serverid=ServerId}) ->
    Existing = read(PKey),
    Modified = cluster_metadata_object:modify(Existing, Context, ValueOrFun, ServerId),
    store(PKey, Modified, State).

read_merge_write(PKey, Obj, State) ->
    Existing = read(PKey),
    case cluster_metadata_object:reconcile(Obj, Existing) of
        false -> {false, State};
        {true, Reconciled} ->
...
store({FullPrefix, Key}=PKey, Metadata, State) ->
    ...
    Objs = [{Key, Metadata}],
    Hash = cluster_metadata_object:hash(Metadata),
    ets:insert(ets_tab(FullPrefix), Objs),
    cluster_metadata_hashtree:insert(PKey, Hash),
...
default_data_root() ->
    case application:get_env(cluster_metadata, platform_data_dir) of
